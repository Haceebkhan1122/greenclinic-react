...
import './examinationConsultNowTab.scss';
import { useState, useEffect } from 'react';
import API from '../../../services/httpInstance';
import { toast } from 'react-toastify';

const ExaminationConsultNowTab = ({ appointmentId, isDiagnosis, vitalList, defaultVitals, setDefaultVitals, examinationList, setVitalList, patientId, examslug, clinicId, doctorId, setExamslug, vitalArray, setVitalArray, setDefaultVitalArray, defaultVitalArray }) => {
                                ...
                                {examinationList?.map((item) => (
                                    <div key={item?.id}>
                                        {item?.field_type === 'textarea' ? (
                                            <div className="singleTextArea pb-2">
                                                <label htmlFor="">{item?.name}</label>
                                                <textarea
                                                    name={item?.name}
                                                    placeholder={item?.placeholder}
                                                    value={item?.value || item?.consultation_examinations_slug?.[0]?.examination_value}
                                            ...
                                            <div className="single_field customSelect">
                                                ...
                                                <Form.Select
                                                    name={item?.name}
                                                    value={item?.value || item?.consultation_examinations_slug?.[0]?.examination_value} // Safely accessing the first element of consultation_examinations_slug
                                                    onChange={(e) => handleValueChange(item?.slug, e.target.value)}
                                                >
                                                        ...
                                                        <div className="singleTick customTickCheck" key={list?.id}>
                                                            <label htmlFor="tick">
                                                                <input
                                                                    type="checkbox"
                                                                    id="tick"
                                                                    checked={item?.consultation_examinations_slug[0]?.examination_value?.includes(list?.value) || item?.value?.includes(list?.value)}
                                                        ...
                                                        <div className="single customRadioo" key={list?.id}>
                                                            <div className="wrapeInp">
                                                                <input
                                                                    ...
                                                                    id={list?.value}
                                                                    name={item?.slug}
                                                                    checked={item?.consultation_examinations_slug?.[0]?.examination_value === list?.value || item?.value === list?.value} // Pre-fill with examination_value
                                                        ...
                                                        <div className="single customRadioo" key={list?.id}>
                                                            <div className="wrapeInp">
                                                                <input
                                                                    ...
                                                                    id={list?.value}
                                                                    name={item?.slug}
                                                                    checked={item?.consultation_examinations_slug?.[0]?.examination_value === list?.value || item?.value === list?.value} // Check the correct state value
                                                                    onChange={() => handleRadioChange(item?.slug, list?.value)}
                                                                />
                                                                <span></span>
                                                            </div>
                                            ...
                                            <div className="dropdown">
                                                <div className="dagnosis-select">
                                                    <div onClick={toggleDropdown} className="tabslidedown">
                                                        <span>Search for {selectCategory == 'indications' ? 'indications' : 'Past Medical History'}</span>
                                                    </div>
...
export default ExaminationConsultNowTab;
