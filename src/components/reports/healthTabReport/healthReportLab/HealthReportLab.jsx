...
const HealthReportMedications = () => {
    ...
    const [lebFilterShow, setLabFilterShow] = useState("")
    const handleLabFilterClose = () => setLabFilterShow(false);
    const handleLabFilterShow = () => setLabFilterShow(true);
    ...
    const getMedicneReports = async () => {
        try {
            ...
            const url = queryParams.toString()
                ? `/reports/health-lab-list?page=${currentPage}&${queryParams.toString()}`
                : `/reports/health-lab-list?page=${currentPage}`;
    ...
    const filteredData = searchTerm
        ? medicineTableData?.data?.filter((item) =>
            item?.lab_title?.toLowerCase().includes(searchTerm.toLowerCase()) ||
            item?.vendor?.toLowerCase().includes(searchTerm.toLowerCase())
        )
        : medicineTableData?.data;

    async function downloadAPI() {
        fetch(`${baseUrl}/reports/health-lab-download`, {
            method: 'GET',
            headers: {
            ...
            .then((data) => {
                ...
                a.href = fileUrl;
                a.download = "labs.csv"; // Ensure CSV format
    ...
    return (
        <WraperLayout>
            <div className="healthReport_medication">
                ...
                {isMobile ?
                    <>
                        <div className="wrapePrint" onClick={handleLabFilterShow}>
                                            ...
                                            <Accordion.Body>
                                                <ul className="servicesTotal">
                                                    ...
                                                    <li className='appointmetList'>
                                                        <h5 className='keyTitle '>  Type </h5>
                                                        <span className="keyBody vitalName"> {item?.type} </span>
                                                    </li>
                                                    <li className='appointmetList'>
                                                    ...
                                                    <li className='appointmetList'>
                                                        ...
                                                        <span className="keyBody vitalName"> {item?.nos_times} </span>
                                                    </li>
                            ...
                            <Table responsive className=''>
                                ...
                                {filteredData?.map((item) => {
                                    return (<>
                                        <tr>
                                            <td> {item?.s_no} </td>
                                            <td> {item?.lab_title}  </td>
                                            <td> {item?.type} </td>
                                            <td> {item?.city} </td>
                                            <td> {item?.clinic} </td>
                                            <td> {item?.nos_times} </td>
                                        </tr>
                                    </>)
                                })}
                                <tbody>
                ...
                <Modal handleDateChange={handleDateChange} show={lebFilterShow} onHide={handleLabFilterClose} centered className="mobileFilterReportModal">
                    <Modal.Body>
                        <span className="crossBtnModal" onClick={handleLabFilterClose}></span>
