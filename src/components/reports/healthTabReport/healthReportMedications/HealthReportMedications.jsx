...
import { Row, Container, Col, Form, Table, Modal, Accordion } from "react-bootstrap"
import WraperLayout from '../../../wraperLayout/WraperLayout'
import { useNavigate } from 'react-router-dom'
import { DatePicker } from 'antd';
import "./healthReportMedications.scss"
...
const HealthReportMedications = () => {
    ...
    const [medicationFilterShow, setMedicationFilterShow] = useState('');
    const handleMedicationFilterClose = () => setMedicationFilterShow(false);
    const handleMedicationFilterShow = () => setMedicationFilterShow(true);
    ...
    const getMedicneReports = async () => {
        try {
            ...
            if (selectedVendor) {
                queryParams.append("vendor", selectedVendor);
            }

            const url = queryParams.toString()
                ? `/reports/medicine-list?page=${currentPage}&${queryParams.toString()}`
                : `/reports/medicine-list?page=${currentPage}`;
    ...
    useEffect(() => {
        getMedicneReports();
    }, [selectedDate, selectedVendor, currentPage]);

    const filteredData = searchTerm
        ? medicineTableData?.data?.filter((item) =>
            item?.med_title?.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    async function downloadAPI() {
        fetch(`${baseUrl}/reports/medicine-download`, {
            method: 'GET',
            headers: {
            ...
            .then((data) => {
                ...
                a.href = fileUrl;
                a.download = "medications.csv"; // Ensure CSV format
    ...
    return (
        <WraperLayout>
            <div className="healthReport_medication">
                ...
                {isMobile ?
                    <>
                        <div className="wrapePrint" onClick={handleMedicationFilterShow}>
                                                ...
                                                <div className='accordHeader tw-w-full tw-flex tw-items-center tw-justify-between'>
                                                    <div className='tw-flex tw-w-full  tw-flex-col tw-justify-center'>
                                                        <div className=' tw-w-full tw-flex tw-items-center tw-justify-between tw-gap-1'>
                                                            <h5>  {item?.med_title}</h5>
                                                            <span className="priceTag"> S.No {item?.s_no}</span>
                                                        </div>
                                            ...
                                            <Accordion.Body>
                                                <ul className="servicesTotal">
                                                    ...
                                                    <li className='appointmetList'>
                                                        <h5 className='keyTitle '>vendor </h5>
                                                        <span className="keyBody vitalName"> {item?.vendor} </span>
                                                    </li>
                                                    <li className='appointmetList'>
                                                        <h5 className='keyTitle '>city </h5>
                                                        <span className="keyBody vitalName"> {item?.city} </span>
                                                    </li>
                                                    <li className='appointmetList'>
                                                        <h5 className='keyTitle '>clinic </h5>
                                                        <span className="keyBody vitalName"> {item?.clinic} </span>
                                                    </li>
                                                    <li className='appointmetList'>
                                                        <h5 className='keyTitle '>Total  </h5>
                                                        <span className="keyBody vitalName"> {item?.total_prescibed} </span>
                                                    </li>
                    ...
                    <>
                        <div className="top-bar-filter">
                            ...
                            <div className="single_field customSelect">
                                <Form.Select aria-label="Default select example" name='vendor' value={selectedVendor} onChange={handleChange}>
                                    <option value={""}>Select Vendor</option>
                                    {vendorList?.length > 0 && vendorList.map((vendor) => (
                            ...
                            <Table responsive className=''>
                                <thead>
                                    <tr>
                                        ...
                                        <th>City</th>
                                        <th>Clinic</th>
                                        <th>No. of the time prescribed</th>
                                    </tr>
                                </thead>
                ...
                <Modal handleDateChange={handleDateChange} show={medicationFilterShow} onHide={handleMedicationFilterClose} centered className="mobileFilterReportModal">
                    <Modal.Body>
                        <span className="crossBtnModal" onClick={handleMedicationFilterClose}></span>
                            ...
                            <div className="wrape_cl">
                                <Row>
                                    ...
                                    <Col xs={12}>
                                        ...
                                        <div className="single_field customSelect">
                                            <label htmlFor="openDate">Vendor</label>
                                            <Form.Select aria-label="Default select example" name='vendor' value={selectedVendor} onChange={handleChange}>
                                                <option value={""}>Select Vendor</option>
                                                {vendorList?.length > 0 && vendorList.map((vendor) => (
                                                    <option key={vendor.id} value={vendor.id}>
