...
import "./healthReportIndications.scss"
...
const HealthReportMedications = () => {
    ...
    const [indicationFilterShow, setIndicationFilterShow] = useState("")
    const handleIndicationFilterClose = () => setIndicationFilterShow(false);
    const handleIndicationFilterShow = () => setIndicationFilterShow(true);
    ...
    const getMedicneReports = async () => {
        try {
            ...
            const url = queryParams.toString()
                ? `/reports/diagnosis-all?page=${currentPage}&${queryParams.toString()}`
                : `/reports/diagnosis-all?page=1`;
            ...
            if (response?.status == 200) {
                ...
                setMedicineTableData(response?.data?.data?.Indications);
                setFilteredData(response?.data?.data?.Indications);
                setIsLoading(false);
            }
        }
        catch (error) {
    ...
    useEffect(() => {
        if (searchTerm) {
            const filtered = medicineTableData?.filter((item) =>
                item?.patientName?.toLowerCase().includes(searchTerm.toLowerCase()) ||
                item?.mrNo?.toString().includes(searchTerm)
            );
            setFilteredData(filtered);
        } else {
            setFilteredData(medicineTableData);
        }
    }, [medicineTableData, searchTerm]);

    async function downloadAPI() {
        fetch(`${baseUrl}/reports/download-current-diagnosis`, {
            method: 'GET',
            headers: {
            ...
            .then((data) => {
                ...
                a.href = fileUrl;
                a.download = "indications.csv"; // Ensure CSV format
    ...
    return (
        <WraperLayout>
            <div className="healthReport_medication">
                ...
                {isMobile ?
                    <>
                        <div className="wrapePrint" onClick={handleIndicationFilterShow}>
                                                    ...
                                                    <div className='tw-flex tw-w-full  tw-flex-col tw-justify-center'>
                                                        <div className=' tw-w-full tw-flex tw-items-center tw-justify-between tw-gap-1'>
                                                            <h5>   {item?.patientName}
                                                                ...
                                                                <span className='d-block'> {item?.age} | {item?.gender}</span>
                                                            </h5>

                                                            <span className="priceTag">{item?.mrNo} </span>
                                                        </div>
                                                    </div>
                                                </div>
                                            </Accordion.Header>
                                            <Accordion.Body>
                                                <ul className="servicesTotal">
                                                    ...
                                                    <li className='appointmetList'>
                                                        ...
                                                        <span className="keyBody vitalName"> Dr. {item?.doctorName} </span>
                                                    </li>
                                                    ...
                                                    <li className='appointmetList'>
                                                        <h5 className='keyTitle '>Indications </h5>
                                                        <span className="keyBody vitalName"> {item?.indication} </span>
                                                    </li>
                ...
                <Modal handleDateChange={handleDateChange} show={indicationFilterShow} onHide={handleIndicationFilterClose} centered className="mobileFilterReportModal">
                    <Modal.Body>
                        <span className="crossBtnModal" onClick={handleIndicationFilterClose}></span>
