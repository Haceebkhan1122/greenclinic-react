...
const ExaminationTabOffline = ({ appointmentId, isDiagnosis, vitalList, defaultVitals, setDefaultVitals, examinationList, setVitalList, patientId, examslug, clinicId, doctorId, setExamslug, vitalArray, setVitalArray, setDefaultVitalArray, defaultVitalArray }) => {
    ...
    const combinedVitals = [
        ...[...vitalList || [], ...defaultVitals || []].filter(
            (item, index, self) =>
                ...
                <Row className='h-100'>
                    <Col lg={4} style={{ borderRight: "1px solid #D3DEE9" }}>
                        ...
                        {combinedVitals.length > 0 && (
                            <ul>
                                {combinedVitals.map((item) => (
                                    <li key={item?.id}>
                                        <label>{item?.title}</label>
                                        <input
                                            type="text"
                                            value={item?.consultation_vital_title?.vitals_value || item?.value || ''}
                                            onChange={(e) => {
                                                const updatedValue = e.target.value;

                                                const isInVitalList = vitalList?.some((v) => v.id === item.id);

                                                if (isInVitalList) {
                                                    setVitalList((prev) =>
                                                        prev.map((i) => (i.id === item.id ? { ...i, value: updatedValue } : i))
                                                    );
                                                    setVitalArray((prev) =>
                                                        prev.map((v) => (v.slug === item.slug ? { ...v, value: updatedValue } : v))
                                                    );
                                                } else {
                                                    setDefaultVitals((prev) =>
                                                        prev.map((i) => (i.id === item.id ? { ...i, value: updatedValue } : i))
                                                    );
                                                    setDefaultVitalArray((prev) =>
                                                        prev.map((v) => (v.slug === item.slug ? { ...v, value: updatedValue } : v))
                                                    );
                                                }
                                            }}
