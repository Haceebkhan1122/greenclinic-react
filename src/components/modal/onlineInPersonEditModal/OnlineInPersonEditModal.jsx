...
import './onlineInPersonEditModal.scss';
import { useSelector } from 'react-redux';
import Loader from '../../loader/Loader';
import { isMobile } from 'react-device-detect';

const OnlineInPersonEditModal = ({ onlineInPersonEdit, setIndicationMessage, userId, indicationMessage, handleOnlineInPersonEditClose, doctorId, getInPersonDataOnline }) => {
  ...
  useEffect(() => {
    ...
    const handleUpdate = async () => {
        try {
            ...
            if(response?.status == 200 || response?.status == 201) {
                ...
                handleOnlineInPersonEditClose();
                getViewDetails(doctorId, "in-person");
            }
            else {
                setIsLoading(false)
                setIndicationMessage(response?.data?.message)
    ...
    useEffect(() => {
        if (userId !== "" && userId !== null && userId !== undefined) {
            getViewDetails(userId, "in-person")
        }
    }, [onlineInPersonEdit])
    ...
    }, [inPersonData, onlineInPersonEdit]);
    ...
    return (
        ...
        <Loader />
        :
        <Modal show={onlineInPersonEdit} onHide={handleOnlineInPersonEditClose} centered className="modalInPersonConsult">
            <Modal.Body>
                {indicationMessage !== "" && <div className="showPoup">
                    {indicationMessage}
                </div>}
                <span className='cross_icc' onClick={handleOnlineInPersonEditClose}></span>
                <Row className='bg_from'>
                    <h2> <span className='back__ico' onClick={handleOnlineInPersonEditClose}></span> Online In-person Consultation </h2>
                                ...
                                <Row>
                                    ...
                                    <Col lg={6} className="mb-3">
                                        <Form.Group>
                                            ...
                                            <Form.Control  min="0" max="100" type="text" placeholder='Enter share price in percentage' name='doctorShare' onChange={handleChangeForm} value={doctorShare} />
                                        </Form.Group>
                                    </Col>
                                    <Col lg={6} className="mb-3">
                                        <Form.Group>
                                            <Form.Label>Clinic Share (%)</Form.Label>
                                            <Form.Control disabled type="text" className='clinicShareInp' placeholder='Auto calculate based on doctor share' name='clinic_share' onChange={handleChangeForm} value={clinicShare} />
                                        </Form.Group>
                                    </Col>
                                    {/* <Col lg={6} className="mb-3">
                                        <Form.Group>
                ...
                {isMobile ?
                    <div className="bottom_bar_btn">
                        <button className='' onClick={handleOnlineInPersonEditClose}>Cancel</button>
...
export default OnlineInPersonEditModal;
